...
ENV ELECTRUM_DIRECTORY=${ELECTRUM_HOME}/.bitcart-arbeth
ENV IN_DOCKER=1
ENV UV_COMPILE_BYTECODE=1
ENV UV_NO_CACHE=1
ENV UV_NO_SYNC=1
ENV ARBETH_HOST=0.0.0.0
LABEL org.bitcart.image=arbeth-daemon
...
RUN apk add git gcc python3-dev musl-dev automake autoconf libtool file git make libffi-dev && \
    ...
    uv sync --frozen --no-dev --group arbeth --group otel && \
